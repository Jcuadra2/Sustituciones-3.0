	...
	<div id="contenido_importar">
		<?php
            ...
            if($fila = $conexion->resultado->fetch_array()){
               $bd_dni=$fila["dni"];
               echo '<h1>El usuario con DNI: '.$bd_dni.'</h1>'; 
            }
            $num = $conexion->n_filas_devueltas();
            if($num>0){
                $conexion2=new conexionBD();
                $conexion2->conectar();
                $consulta = 'UPDATE profesores SET nombre="'.$nombre.'",correo="'.$correo.'" WHERE dni="'.$dni.'";';
                $conexion2->consultar($consulta);
                $afectadas = $conexion2->filas_afectadas();

                if($afectadas>0){
                    echo '<h1> a sido modificado correctamente</h1>';
                }
                else{
                    echo '<h1> no a podido modificarse</h1>';
                }
            }
